...
class StickFigureSprite(Sprite):
    ...
    def move(self):
        ...
        for sprite in self.game.sprites:
            ...
            if top and self.y < 0 and collited_top(co, sprite_co):
                self.y = -self.y
            if bottom and self.y > 0 and collited_bottom(co, sprite_co):
                if self.y < 0:
                    self.y = 0
                bottom = False
                top = False
            if bottom and falling and self.y == 0 and co.y2 < self.game.canvas_height and collited_bottom(1, co, sprite_co):
                falling = False
            if left and self.x < 0 and collited_left(co, sprite_co):
                self.x = 0
                left = False
                if sprite.endgame:
                    self.game.running = False    
            if right and self.x > 0 and collited_right(co, sprite_co):
                self.x = 0
                right = False
                if sprite.endgame:
                    self.game.running = False 
